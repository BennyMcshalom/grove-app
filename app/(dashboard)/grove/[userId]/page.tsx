...
export default function GrovePage() {
  ...
  const RINGS = RINGS_BASE.map((r) => ({
    ...r,
    color: RING_COLORS[r.key][isDark ? "dark" : "light"],
  }));
  ...
  const alreadyConnected =
    bondsData?.some((b) => b.otherUser?.id === userId) ?? false;
  ...
  const hasntFilled = isOwnProfile
    ? "You haven't filled this in yet."
    : `${firstName} hasn't filled this in yet.`;
  const hasntPosted = isOwnProfile
    ? "You haven't posted any log entries yet."
    : `${firstName} hasn't posted any log entries yet.`;

  const uniqueSpaceIds = (
    (grove?.activeSpaces.map((s) => s.space?.slug).filter(Boolean) as
      | string[]
      | undefined) ?? ["career", "learning", "spiritual", "adventure"]
  )
  ...
  const chapter =
    closedChapters[Math.min(ci, Math.max(closedChapters.length - 1, 0))];

  const logForViewer = logEntries.slice(0, 10).map((e) => ({
    date: new Date(e.entryDate).toLocaleDateString("en-US", {
      month: "short",
      day: "numeric",
    }),
    mediaUrl: e.mediaUrl,
  ...
  function getRingContent(key: "inner" | "middle" | "outer"): string | null {
    ...
    return (
      {
        inner: grove.rings.struggling,
        middle: grove.rings.building,
        outer: grove.rings.openTo,
      }[key] ?? null
    );
  }
  ...
  const saveName = async () => {
    ...
    try {
      ...
      qc.setQueryData(
        ["grove", userId],
        (old: typeof grove) =>
          old && { ...old, profile: { ...old.profile, displayName: value } },
      );
  ...
  const startEditRing = () => {
    setRingDraft(getRingContent(active as "inner" | "middle" | "outer") ?? "");
    setEditingRing(true);
  };
  const saveRing = async () => {
    ...
    try {
      if (ring.field === "struggling") {
        ...
        await spacesApi.update(primarySpace.id, {
          stage: ringDraft || undefined,
        });
      }
  ...
  const selectRing = (key: string | null) => {
    setActive(key);
    setEditingRing(false);
  };
  ...
  return (
    <div
      ...
      style={{
        ...
        overflowX: "hidden",
        background:
          "radial-gradient(circle at 50% 38%, var(--surf-high), var(--bg) 70%)",
      }}
    >
      <TopBar
        isLoading={isLoading}
      ...
      <div
        style={{
          ...
          maxWidth: 1100,
          margin: "0 auto",
          padding: "0 clamp(1rem, 4vw, 1.6rem) 3rem",
      ...
      >
        ...
        >
          ...
          <RingDetailPanel
            active={active}
            rings={RINGS}
            getRingContent={getRingContent}
            chapter={chapter}
            isOwnProfile={isOwnProfile}
            primarySpace={primarySpace}
            editingRing={editingRing}
            setEditingRing={setEditingRing}
            ringDraft={ringDraft}
            setRingDraft={setRingDraft}
            savingRing={savingRing}
            stageOptions={stageOptions}
            hasntFilled={hasntFilled}
            firstName={firstName}
            onStartEditRing={startEditRing}
            onSaveRing={saveRing}
            onSelectRing={selectRing}
          />
          ...
          <GroveLogPanel
            possessiveCap={possessiveCap}
            entries={logForViewer}
            isLoading={isLoading}
            logVisible={logVisible}
            hasntPosted={hasntPosted}
            isOwnProfile={isOwnProfile}
