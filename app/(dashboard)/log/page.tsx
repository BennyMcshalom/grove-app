...
function GMark({ size = 20, color = '#fff', bg }: { size?: number; color?: string; bg?: string }) {
  return (
    <div style={{ width: size, height: size, borderRadius: size * 0.32, background: bg || 'rgba(255,255,255,.18)',
...
function MemoryLightbox({ entries, startIndex = 0, space, onClose }: {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 7200, background: 'rgba(20,14,8,.62)', backdropFilter: 'blur(5px)',
      display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.5rem' }} onClick={onClose}>
      {canPrev && (
        <button onClick={e => { e.stopPropagation(); prev(); }}
          style={{ position: 'absolute', left: 'max(1rem, calc(50% - 260px))', top: '50%', transform: 'translateY(-50%)',
      ...
      {canNext && (
        <button onClick={e => { e.stopPropagation(); next(); }}
          style={{ position: 'absolute', right: 'max(1rem, calc(50% - 260px))', top: '50%', transform: 'translateY(-50%) scaleX(-1)',
            width: 46, height: 46, borderRadius: '50%', background: 'rgba(255,255,255,.16)', backdropFilter: 'blur(6px)',
            display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2 }}>
          <Icon name="back" size={20} stroke="#fff"/>
        </button>
      ...
      <div className="swap-in" key={idx} onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}
        ...
        <div style={{ position: 'relative', height: 320 }}>
          ...
          <button onClick={onClose} style={{ position: 'absolute', top: 12, right: 12, width: 34, height: 34, borderRadius: '50%',
          ...
          <div style={{ position: 'absolute', left: 14, bottom: 12, display: 'flex', alignItems: 'center', gap: '.5rem' }}>
            <span style={{ width: 30, height: 30, borderRadius: 9, background: 'rgba(255,255,255,.92)',
        ...
        <div style={{ padding: '1.4rem 1.5rem 1.6rem' }}>
          ...
          {entries.length > 1 && (
            <div style={{ display: 'flex', gap: 5, marginTop: '1.1rem', justifyContent: 'center' }}>
              {entries.map((_, i) => (
                <span key={i} style={{ width: i === idx ? 16 : 6, height: 6, borderRadius: 100,
  ...
  if (posted && !editing) {
    return (
      <div style={{ display: 'flex', justifyContent: 'center' }}>
        <div className="log-card-shadow" style={{ width: 316, maxWidth: '100%', borderRadius: 24, overflow: 'hidden',
          ...
          <div style={{ padding: '1.3rem 1.4rem', textAlign: 'center' }}>
            <div style={{ display: 'inline-block', animation: 'stamp .6s ease both', border: '2.5px solid var(--ember)',
    ...
    <div style={{ display: 'flex', justifyContent: 'center' }}>
      ...
      <div className="log-card-shadow" style={{ width: 316, maxWidth: '100%', borderRadius: 24, overflow: 'hidden',
        ...
        <div style={{ padding: '1.5rem 1.5rem 1.2rem' }}>
          ...
          <p className="serif" style={{ fontSize: '1.5rem', fontWeight: 600, fontStyle: 'italic', color: 'var(--ember)',
            ...
            <div className="swap-in">
              {imgSrc ? (
                ...
                <button onClick={() => fileRef.current?.click()}
                  style={{ width: '100%', height: 120, borderRadius: 14, border: '1.5px dashed var(--border-2)', background: 'var(--surf-low)',
              ...
              <textarea autoFocus value={text} onChange={e => setText(e.target.value.slice(0, 140))} placeholder="One honest line about today…"
                style={{ width: '100%', minHeight: 74, resize: 'none', padding: '.8rem .9rem', fontSize: '1rem', lineHeight: 1.5,
...
function StyleA({ entries, onOpen }: { entries: LogEntry[]; onOpen: (i: number) => void }) {
  ...
  return (
    <div style={{ borderRadius: 26, padding: '1.9rem 1.2rem 1.6rem', background: 'linear-gradient(165deg, #F6C078, #E08A3C 60%, #B5611E)', boxShadow: 'inset 0 1px 0 rgba(255,255,255,.3)' }}>
      ...
      <div style={{ marginTop: '1.2rem', display: 'flex', alignItems: 'center', gap: '.9rem', background: 'rgba(255,255,255,.22)',
    ...
    <div style={{ borderRadius: 28, padding: '2.4rem 1.2rem 2.6rem', background: 'linear-gradient(170deg, #FBFBFA, #ECEBE8)' }}>
      <div style={{ position: 'relative', height: 340, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        {[2, 1].map(d => entries[i + d] && (
          <div key={d} style={{ position: 'absolute', width: 230 - d * 18, height: 288 - d * 12, borderRadius: 22, background: '#fff',
        ...
        {(() => {
          ...
          return (
            <button onClick={() => onOpen(Math.min(i, entries.length - 1))} className="swap-in" key={i}
              style={{ position: 'relative', zIndex: 3, width: 240, height: 300, borderRadius: 22, overflow: 'hidden', background: '#fff',
                boxShadow: '0 26px 52px -18px rgba(40,36,30,.5)', textAlign: 'left' }}>
              <div style={{ height: 184, position: 'relative' }}>
                {e.media
                  ? <img src={e.media} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }}/>
                  : <div style={{ width: '100%', height: '100%', background: 'var(--surf-high)' }}/>}
                <div style={{ position: 'absolute', right: 12, bottom: -18, width: 40, height: 40, borderRadius: '50%', background: '#fff',
  ...
  return (
    <div style={{ borderRadius: 28, padding: '2.1rem 1.2rem 1.6rem', background: '#FAFAF8' }}>
      <div style={{ position: 'relative', height: 460, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        {[2, 1].map(d => entries[i + d] && (
          <div key={d} style={{ position: 'absolute', width: 300 - d * 20, height: 416 - d * 16, borderRadius: 24, background: '#15110D',
        ...
        <div style={{ display: 'flex', gap: 3, background: 'var(--surf-high)', borderRadius: 100, padding: 3 }}>
          {LOG_STYLES.map(([id, l]) => (
            <button key={id} onClick={() => onStyleChange(id)}
              style={{ padding: '.4rem .85rem', borderRadius: 100, fontSize: '.8rem', fontWeight: 600,
...
function LogViewer({ log, onClose }: { log: OtherLog; onClose: () => void }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 7100, background: 'rgba(26,18,10,.55)', backdropFilter: 'blur(4px)',
      display: 'flex', justifyContent: 'center', padding: '4vh 1rem' }}
      onClick={onClose}>
      <div className="scroll fade-in" style={{ width: 'min(480px, 96vw)', maxHeight: '92vh', overflowY: 'auto',
        background: 'var(--cream)', borderRadius: 24, boxShadow: 'var(--shadow-lg)' }}
        onClick={e => e.stopPropagation()}>
        <div style={{ position: 'sticky', top: 0, zIndex: 2, background: 'var(--cream)', borderBottom: '1px solid var(--border)',
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1.1rem' }}>
        {logs.map((log, li) => (
          <article key={li} className="card" style={{ overflow: 'hidden' }}>
            ...
            <button onClick={() => onOpen(log)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '.4rem',
            ...
            {userSpaces.map(id => {
              ...
              return (
                <button key={id} onClick={() => setSpaceSlug(id)} className="chip"
                  style={{ cursor: 'pointer', flexShrink: 0, display: 'inline-flex', alignItems: 'center', gap: '.4rem',
