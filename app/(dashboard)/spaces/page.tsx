    ...
    <AppShell title="My Spaces" right={right}>
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '0 1.6rem 3rem' }}>
        ...
        <div id="spaces-dir">
          ...
          <div className="spaces-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
            {dirSpaces.map(s => (
              <SpaceDirectoryCard key={s.id} s={s} isOpening={opening === s.id}
                chapter={chapter} setChapter={setChapter} submitting={openSpace.isPending}
                onStartOpen={() => { setOpening(s.id); setChapter(''); }}
                onCancel={() => { setOpening(null); setChapter(''); }}
