...
export function NearbyCard({
  ...
  const [waveState, setWaveState] = useState<"idle" | "sending" | "sent">(
    "idle",
  );
  const sharedSpaces = [...new Set(person.spaces)].filter((s) =>
    mySpaces.includes(s),
  );
      ...
      <div style={{ display: "flex", alignItems: "center", gap: ".85rem" }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {hasShared ? (
            ...
            >
              {sharedSpaces.slice(0, 3).map((s, i) => (
                <SpaceIcon key={`${s}-${i}`} spaceId={s} size={12} />
              ))}
              <span
                style={{
                  fontSize: ".72rem",
          ...
          {waveState === "sending" ? (
            ...
            <span style={{ display: "flex", alignItems: "center", gap: 4 }}>
              <Icon
                name="wave"
                size={13}
                stroke={waveState === "sent" ? "#fff" : "var(--ember)"}
                sw={1.6}
              />
