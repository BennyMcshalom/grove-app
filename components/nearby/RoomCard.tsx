...
export function RoomCard({
  ...
  const hoursLeft = Math.max(
    0,
    Math.round((new Date(room.expiresAt).getTime() - Date.now()) / 3_600_000),
  );
      ...
      <div style={{ display: "flex", alignItems: "flex-start", gap: ".8rem" }}>
        <div
          style={{
            ...
            height: 42,
            borderRadius: 12,
            background: joined ? "var(--green-dim)" : "var(--ember-dim)",
            display: "flex",
            alignItems: "center",
        ...
        >
          <Icon
            name="pin"
            size={19}
            stroke={joined ? "var(--green)" : "var(--ember)"}
          />
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontWeight: 600, fontSize: ".94rem" }}>
            {room.gatheringTag}
          </div>
          <div
            style={{
