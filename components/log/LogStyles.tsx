...
export function StyleB({ entries, onOpen }: { entries: LogEntry[]; onOpen: (i: number) => void }) {
...
// ── Style C — Card (dark) ──
export function StyleC({ entries, space, onOpen }: { entries: LogEntry[]; space: Space; onOpen: (i: number) => void }) {
  const [i, go] = useCarousel(entries.length);
  if (!entries.length) return null;
  const e = entries[Math.min(i, entries.length - 1)] || entries[0];
  return (
