...
export function WeeklyReflection({ weeklyQ, ans, setAns, saved, setSaved, onSave }: {
  weeklyQ: string;
  ans: string; setAns: (v: string) => void;
  ...
  return (
    <div className="card" style={{ padding: '1.6rem' }}>
      ...
      <div style={{ display: 'flex', gap: '.6rem' }}>
        <button onClick={() => onSave(false)} className="btn"
          style={{ background: 'var(--sage)', color: '#fff', flex: 1 }}>
          {saved ? 'Update reflection' : 'Save privately'}
        </button>
        <button onClick={() => onSave(true)} className="btn btn-ghost" style={{ flex: 1 }}>
          Share to my space
        </button>
      </div>
    </div>
