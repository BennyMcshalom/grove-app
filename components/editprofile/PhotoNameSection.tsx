...
export function PhotoNameSection({ name, setName, location, setLocation, aura, avatarUrl, uploading,
  locating, onPickPhoto, onDetectLocation, userName }: {
  name: string; setName: (v: string) => void;
  location: string; setLocation: (v: string) => void;
  aura: AuraKey; avatarUrl: string | null | undefined; uploading: boolean;
  locating: boolean;
  onPickPhoto: () => void; onDetectLocation: () => void;
  userName: string;
}) {
  return (
    <Section label="Photo & name">
      <div style={{ display: 'flex', alignItems: 'center', gap: '1.1rem' }}>
        <div style={{ position: 'relative', flexShrink: 0 }}>
          <Avatar name={name || userName} size={76} aura={aura} avatarUrl={avatarUrl}/>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', gap: '.7rem', alignItems: 'center', marginTop: '.5rem' }}>
            <button onClick={onPickPhoto} style={{ fontSize: '.8rem', color: 'var(--ember)', fontWeight: 500 }}>
              Upload a photo
            </button>
          </div>
        </div>
      ...
      <div style={{ marginTop: '1rem' }}>
        ...
        <div style={{ position: 'relative' }}>
          ...
          <button type="button" onClick={onDetectLocation} disabled={locating} title="Use current location"
