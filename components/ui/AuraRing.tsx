...
import styles from './AuraRing.module.css';
...
export function AuraRing({ aura, size }: AuraRingProps) {
  ...
  if (aura === 'focus') return (
    <div className={styles.ring} style={{ width: out + 3, height: out + 3, transform: 'translate(-50%,-50%)',
  ...
  if (aura === 'transition') return (
    <div className={styles.ring} style={{ width: out, height: out, transform: 'translate(-50%,-50%)',
  ...
  if (aura === 'active') return (
    <>
      {[0,1,2,3,4].map(i => (
        <span key={i} className={styles.ring} style={{ width: 5, height: 5, marginLeft: -2.5, marginTop: -2.5,
  ...
  if (aura === 'open') return (
    <>
      ...
      <div className={styles.ring} style={{ width: size + 2, height: size + 2, transform: 'translate(-50%,-50%)',
