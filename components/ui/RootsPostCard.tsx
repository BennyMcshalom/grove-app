  ...
  const saveEdit = async () => {
    try {
      await updatePost.mutateAsync({
        id: postId,
        data: { doing: editDoing, honestThing: editHonest },
      });
      setEditing(false);
      toast("Post updated.");
    } catch {
      ...
      >
        ...
        >
          <Avatar
            ...
            avatarUrl={post.anon ? undefined : post.avatarUrl}
          />
        </button>
        ...
        {showViewGrouv && !post.anon && post.userId && (
          <button
            onClick={() => router.push(`/grove/${post.userId}`)}
          ...
          >
            {isOwn &&
              menuRow("Edit Post", () => {
                setMenu(false);
                setEditing(true);
                setEditDoing(post.doing);
                setEditHonest(post.honest);
              })}
            {menuRow("Send to a Bond", () => {
              setMenu(false);
              setSavingToBond(true);
            })}
      ...
      >
        <button
          ...
          style={{
            ...
            fontSize: ".84rem",
            fontWeight: 500,
            color: rooted ? "#fff" : "var(--ember-deep)",
            background: rooted ? "var(--ember)" : "var(--ember-pale)",
            transition: "all .15s",
          }}
        >
          <Icon
            name="sprout"
            size={17}
            stroke={rooted ? "#fff" : "var(--ember-deep)"}
          />{" "}
          Root {roots}
        </button>
        <button
          ...
          style={{
            ...
            fontSize: ".84rem",
            fontWeight: 500,
            color: showC ? "var(--slate)" : "var(--ink-2)",
            background: showC ? "var(--slate-dim)" : "var(--surf-high)",
          }}
        >
          <Icon
            name="comment"
            size={16}
            stroke={showC ? "var(--slate)" : "var(--ink-2)"}
          />{" "}
