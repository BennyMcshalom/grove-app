...
export function SaveToBondModal({
  postId,
  onClose,
}: {
  postId: string;
        ...
        <div style={{ padding: ".6rem 1rem 1.2rem" }}>
          {isLoading ? (
            ...
            connections.map((b) => {
              ...
              const firstName =
                b.otherUser?.displayName?.split(" ")[0] ?? "them";
              return (
                <button
                  key={b.id}
                  onClick={() => send(b.id, firstName)}
                ...
                >
                  <Avatar
                    ...
                    aura={b.otherUser?.aura ?? undefined}
                  />
                  <span style={{ flex: 1, fontWeight: 500, fontSize: ".9rem" }}>
                    {b.otherUser?.displayName ?? "Someone"}
                  </span>
