...
function platformUrl(id: PlatformId, shareText: string, shareUrl: string) {
  ...
  switch (id) {
    ...
    case "email":
      return `mailto:?subject=${encodeURIComponent("Something on Grouv")}&body=${t}%20${u}`;
  }
}
...
export function ShareModal({
  ...
  const name = post.anon
    ? "A connection in your space"
    : post.name || "Someone";
        ...
        >
          {PLATFORMS.map((p) => (
            <button
              key={p.id}
              onClick={() =>
                openExternal(platformUrl(p.id, shareText, shareUrl))
              }
              style={{
                display: "flex",
            ...
            >
              ...
              >
                {p.id === "whatsapp" && (
                  <Icon name="phone" size={19} stroke="#fff" sw={2} />
                )}
                {p.id === "x" && (
                  <span
                    style={{
                ...
                {p.id === "email" && (
                  <Icon name="envelope" size={18} stroke="#fff" sw={2} />
                )}
              </span>
