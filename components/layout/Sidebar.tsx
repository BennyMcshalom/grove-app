...
export function Sidebar() {
  ...
  return (
    <aside style={{
      width: 262, flexShrink: 0, background: 'var(--white)',
        ...
        {NAV.map(item => {
          ...
          return (
            <Link key={item.id} href={item.href}
              style={{
                display: 'flex', alignItems: 'center', gap: '.75rem', padding: '.62rem .7rem',
      ...
      <div style={{ borderTop: '1px solid var(--border)', paddingTop: '.7rem', display: 'flex', flexDirection: 'column', gap: 6 }}>
        ...
        <Link href="/deep-focus" style={{
          display: 'flex', alignItems: 'center', gap: '.6rem',
          padding: '.55rem .7rem', borderRadius: 100, fontSize: '.8rem', color: 'var(--ink-2)',
