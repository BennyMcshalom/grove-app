...
export function QuestionStep({ spaceId, spaceName, step, total, label, placeholder, value, onChange, onContinue, onSkip, continueLabel }: {
  spaceId: string; spaceName: string; step: number; total: number;
  label: string; placeholder: string; value: string; onChange: (v: string) => void;
  onContinue: () => void; onSkip: () => void; continueLabel: string;
}) {
  return (
    <>
      <div className="label-mono" style={{ marginBottom: '1rem', display: 'flex',
        alignItems: 'center', justifyContent: 'center', gap: '.5rem' }}>
        <SpaceIcon spaceId={spaceId} size={12}/> {spaceName} · Question {step} of {total}
      </div>
      ...
      <NavRow onSkip={onSkip} onContinue={onContinue} continueLabel={continueLabel}/>
    </>
  );
}
