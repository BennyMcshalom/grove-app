...
// ── Final step: Summary ──
export function SummaryStep({ spaceId, spaceName, openedAt, closedAt, monthsStr, bonds, answers, extras, held, closing, onSave }: {
  spaceId: string; spaceName: string; openedAt: Date | null; closedAt: Date; monthsStr: string | null;
  bonds: BondRecord[]; answers: string[]; extras: string[]; held: boolean; closing: boolean; onSave: () => void;
}) {
  return (
    <>
      ...
      <div style={{ height: 64, marginTop: '2rem' }}>
        {held ? (
          <button className="btn btn-ghost btn-lg btn-pill fade-in"
            disabled={closing} onClick={onSave}
