...
export function GroupsView({ groups }: { groups: Group[] }) {
  ...
  const visible = groups.filter((g) => {
    if (adminMode && g.myRole !== "admin") return false;
    if (!needle) return true;
    return `${g.title} ${g.label ?? ""} ${g.description ?? ""}`.toLowerCase().includes(needle);
  });
        ...
        <div className="mx-auto flex w-full max-w-[724px] flex-col gap-4 pb-10">
          ...
          {visible.length === 0 ? (
            ...
            <EmptyState
              ...
              body={
                adminMode
                  ? "You're not running any groups yet"
                  : needle
                    ? "No groups match that search"
                    : "It seems there are no Group added yet"
              }
              action={
                <button
                  type="button"
